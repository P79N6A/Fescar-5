...
public class AsyncWorker implements ResourceManagerInbound {
    ...
    private void doBranchCommits() {
        ...
        LOGGER.info("####################     AsyncWorker#doBranchCommits    Iterator:{}", iterator.toString());
        for (Map.Entry<String, List<Phase2Context>> entry : mappedContexts.entrySet()) {
            Connection conn = null;
            try {
                try {
//                    DataSourceProxy dataSourceProxy = DataSourceManager.get().get(entry.getKey());
                    DataSourceProxy dataSourceProxy = ((DataSourceManager)DefaultResourceManager.get().getResourceManager(BranchType.AT)).get(entry.getKey());
                ...
                for (Phase2Context commitContext : contextsGroupedByResourceId) {
                    try {
                        LOGGER.info("####################     AsyncWorker#deleteUndoLog    Xid:{}, BranchId:{}",
                                commitContext.xid, commitContext.branchId);
